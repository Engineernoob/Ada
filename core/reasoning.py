...
from neural.encoder import TextEncoder, LanguageEncoder
...
class RewardEngine:
    ...
    def compute(self, user_feedback: Optional[str], user_input: str, ada_output: str) -> float:
        if user_feedback:
...
class ReasoningEngine:
    def __init__(self, checkpoint_path: Optional[Path] = None, use_language_encoder: bool = True,
                 use_memory: bool = True, use_persona: bool = True) -> None:
        ...
        default_path = Path(__file__).resolve().parents[1] / "storage" / "checkpoints" / "ada_core.pt"
        ...
        self.use_memory = use_memory and get_setting("memory", "episodic_enabled", default=True)
        if self.use_memory:
        ...
        self.use_persona = use_persona and get_setting("persona", "enabled", default=True)
    ...
    def generate_with_metrics(self, user_input: str) -> GenerationResult:
            ...
            if context_text:
                ...
                if hasattr(self.encoder, 'model'):  # LanguageEncoder
                    ...
                    embedding = torch.tensor(
                        self.encoder.encode(context_input), dtype=torch.float32, device=self.device
                    )
            else:
        ...
        memory_id = 0
        if self.use_memory and self.memory_store:
            memory_id = self.memory_store.store(user_input, text, 0.0, original_embedding)
    ...
    def get_persona_stats(self) -> dict:
        ...
        return {
            ...
            "last_update": time.ctime(persona_stats.update_time) if persona_stats.update_time > 0 else "Never"
        }
