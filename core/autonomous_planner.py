...
class AutonomousPlanner:
    ...
    def infer_and_plan(self, trigger_after_conversation: bool = True) -> Dict[str, Any]:
        ...
        if (
            ...
            and hasattr(self.reasoning_engine, "persona")
            and self.reasoning_engine.persona
        ):
            persona_summary = self.reasoning_engine.persona.get_persona_summary()
            persona_intents = self.intent_engine.infer_from_persona(
                persona_summary,
        ...
        for intent in intents[:2]:  # Limit to top 2 intents to avoid overload
            ...
            if plan.confidence > 0.5 and plan.priority > 0.6:
                ...
                print(
                    f"⏸️ Plan confidence ({plan.confidence:.2f}) or priority ({plan.priority:.2f}) too low for automatic execution"
                )
                results.append(
                    {
                        "intent": intent,
    ...
    def plan_from_command(self, goal: str, category: str = "action") -> Dict[str, Any]:
        ...
        intent = Intent(goal=goal, priority=0.8, category=category, confidence=0.9)
        ...
        return {"plan": plan, "intent": intent, "status": "plan_created"}
    ...
    def execute_plan(self, plan_id: str) -> Dict[str, Any]:
        ...
        if not target_plan:
            return {
                "status": "error",
                "error": f"Plan {plan_id} not found or already executed",
            }
