...
class IntentEngine:
    ...
    def infer(
        self, dialogue_history: List[str], recent_context: Optional[str] = None
    ) -> List[Goal]:
        """
    ...
    def _calculate_confidence(self, match: re.Match, utterance: str) -> float:
        ...
        if any(
            word in utterance.lower() for word in ["please", "can", "could", "would"]
        ):
            confidence += 0.1
    ...
    def infer_from_persona(self, persona_summary: str, context: str) -> List[Goal]:
        ...
        if "curious" in persona_summary.lower():
            if any(
                word in context.lower() for word in ["question", "unclear", "wonder"]
            ):
                goals.append(
                    Goal(
        ...
        if "helpful" in persona_summary.lower():
            if any(
                word in context.lower() for word in ["problem", "issue", "challenge"]
            ):
                goals.append(
                    Goal(
        ...
        if "analytical" in persona_summary.lower():
            if any(
                word in context.lower() for word in ["data", "information", "results"]
            ):
                goals.append(
                    Goal(
