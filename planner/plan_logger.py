...
class PlanLogger:
    ...
    def __init__(self, db_path: Optional[Path] = None) -> None:
        if db_path is None:
            db_path = Path(get_setting("paths", "conversations_db", default="storage/plans.db"))
    ...
    def create_tables(self) -> None:
        ...
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_plans_status ON plans(status)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_plans_created_at ON plans(created_at)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_plan_steps_plan_id ON plan_steps(plan_id)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_executions_plan_id ON executions(plan_id)")
    ...
    def log_execution(self, execution_result, plan_id: str, reward: Optional[float] = None) -> None:
    ...
    def update_plan_status(self, plan_id: str, status: str) -> None:
        ...
        """, (status, time.time() if status in ["completed", "failed", "aborted"] else None, plan_id))
        self.conn.commit()
    
    def update_goal_status(self, goal_id: int, status: str, plan_id: Optional[str] = None) -> None:
    ...
    def get_plan_history(self, limit: int = 50, status: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def get_plan_details(self, plan_id: str) -> Optional[Dict[str, Any]]:
        ...
        # Get plan steps
        cursor = self.conn.execute("SELECT * FROM plan_steps WHERE plan_id = ? ORDER BY timestamp", (plan_id,))
        step_rows = cursor.fetchall()
        
        # Get executions
        cursor = self.conn.execute("SELECT * FROM executions WHERE plan_id = ? ORDER BY timestamp", (plan_id,))
    ...
    def get_planning_stats(self) -> Dict[str, Any]:
        ...
        # Execution stats
        cursor = self.conn.execute("SELECT AVG(execution_time), COUNT(*) FROM executions")
        exec_stats = cursor.fetchone()
        avg_execution_time = exec_stats[0] or 0.0
        total_executions = exec_stats[1] or 0
        
        # Success rates
        cursor = self.conn.execute("SELECT AVG(success), COUNT(*) FROM executions WHERE success = 1")
