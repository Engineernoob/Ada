...
class MetaPersona:
    ...
    def __init__(
        self, model_name: str = "all-MiniLM-L6-v2", persona_path: Optional[Path] = None
    ) -> None:
        if persona_path is None:
            persona_path = (
                Path(get_setting("paths", "checkpoints", default="storage/checkpoints"))
                / "persona_vector.pt"
            )
        ...
        self.drift_threshold: float = get_setting(
            "persona", "drift_threshold", default=0.3
        )
    ...
    def build_from_transcripts(self, ada_responses: List[str]) -> torch.Tensor:
        ...
        if SENTENCE_TRANSFORMERS_AVAILABLE and self.encoder is not None:
            ...
            try:
                ...
                embeddings = [
                    text_encoder.encode(response) for response in ada_responses
                ]
                new_vector = torch.tensor(
                    np.mean(embeddings, axis=0), dtype=torch.float32
                )
            except ImportError:
    ...
    def apply_bias(
        self, embedding: torch.Tensor, weight: Optional[float] = None
    ) -> torch.Tensor:
    ...
    def calculate_drift(self) -> float:
        ...
        similarity = cosine_similarity(
            self.vector.unsqueeze(0), self.previous_vector.unsqueeze(0)
        ).item()
    ...
    def needs_update(self, dialog_count: int) -> bool:
        ...
        return dialog_count >= update_interval and get_setting(
    ...
    def analyze_persona(self) -> PersonaStats:
        ...
        self._persona_stats = PersonaStats(
            tone=tone, phrasing=phrasing, drift=drift, update_time=self.last_update_time
        )
    ...
    def get_persona_summary(self) -> str:
        ...
        if stats.drift > self.drift_threshold:
            summary_lines.append(
                "⚠️  High drift detected - persona may need recalibration"
            )
