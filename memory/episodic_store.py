...
warnings.filterwarnings(
    "ignore",
    message=".*not writeable.*does not support non-writable tensors.*",
...
class EpisodicStore:
    ...
    def __init__(self, db_path: Optional[Path] = None) -> None:
        if db_path is None:
            db_path = Path(
                get_setting("memory", "db_path", default="storage/conversations.db")
            )
    ...
    def create_table(self) -> None:
        ...
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_reward ON episodic_memory(reward)"
        )
        self.conn.commit()
    ...
    def retrieve(
        self, query_embedding, top_k: int = 3, min_similarity: float = 0.1
        ...
        for row_id, user_input, ada_response, emb_blob, reward in rows:
            ...
            similarity = cosine_similarity(
                query_embedding, stored_emb.unsqueeze(0)
            ).item()
    ...
    def get_context_string(
        self, query_embedding: torch.Tensor, max_tokens: int = 100
    ) -> str:
        """
    ...
    def update_reward(self, memory_id: int, reward: float) -> None:
        ...
        self.conn.execute(
            "UPDATE episodic_memory SET reward = ? WHERE id = ?", (reward, memory_id)
        )
        self.conn.commit()
    ...
    def get_stats(self) -> dict:
        ...
        cursor = self.conn.execute(
            "SELECT AVG(reward), MAX(reward), MIN(reward) FROM episodic_memory"
        )
    ...
    def cleanup_old_memories(self, days: int = 30) -> int:
        ...
        cursor = self.conn.execute(
            "DELETE FROM episodic_memory WHERE timestamp < ?", (cutoff_time,)
        )
