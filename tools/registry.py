...
class ToolRegistry:
    ...
    def _file_operations(self, action: str, **kwargs) -> ToolResult:
        ...
        try:
            if action == "read":
                ...
                if not path:
                    return ToolResult(False, error="No path provided for read operation")
                ...
                if not path:
                    return ToolResult(False, error="No path provided for write operation")
                ...
                if not source or not dest:
                    return ToolResult(False, error="Source and destination required for copy operation")
                ...
                if not source or not dest:
                    return ToolResult(False, error="Source and destination required for move operation")
    ...
    def _web_search(self, action: str, **kwargs) -> ToolResult:
        ...
        try:
            if action == "search":
                ...
                return ToolResult(False, error=f"Unknown web search operation: {action}")
    ...
    def _summarize_text(self, action: str, **kwargs) -> ToolResult:
        ...
        try:
            if action == "run":
                ...
                if not text_input:
                    return ToolResult(False, error="No input text provided for summarization")
                ...
                if not text_input:
                    return ToolResult(False, error="No input text provided for extraction")
                ...
                if len(important_sentences) < 3:
                    remaining = [s.strip() for s in sentences if s.strip() and s.strip() not in important_sentences]
                    important_sentences.extend(remaining[:3-len(important_sentences)])
                ...
                if not text_input:
                    return ToolResult(False, error="No input text provided for condensation")
    ...
    def _note_operations(self, action: str, **kwargs) -> ToolResult:
        ...
        try:
            if action == "write":
                ...
                note_id = kwargs.get("id", "")  # If ID provided, append to existing note
                ...
                for category, notes in categories.items():
                    organized_summary += f"## {category.title()} ({len(notes)} notes)\n\n"
                    for note in notes[-5:]:  # Show last 5 notes per category
                        organized_summary += f"- {note['date']}: {note['text'][:100]}...\n"
                    organized_summary += "\n"
                ...
                return ToolResult(True, data=f"Organized journal into {len(categories)} categories")
    ...
    def _analyze_data(self, action: str, **kwargs) -> ToolResult:
        ...
        try:
            if action == "evaluate":
                data = kwargs.get("input", "") or kwargs.get("data", "") or kwargs.get("topic", "")
                ...
                evaluation = {
                    ...
                    "complexity": "simple" if len(str(data)) < 100 else "medium" if len(str(data)) < 500 else "complex",
                ...
                if not data:
                    return ToolResult(False, error="No data provided for interpretation")
                
                # Simple interpretation
                interpretation = f"Analysis of provided content:\n"
                ...
                interpretation += f"- Estimated complexity: " + ("simple" if len(str(data)) < 100 else "medium" if len(str(data)) < 500 else "complex")
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        stop_words = {"the", "a", "an", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with", "by", "is", "are", "was", "were", "be", "been", "have", "has", "had", "do", "does", "did", "will", "would", "could", "should", "can", "may", "might", "this", "that", "these", "those", "i", "you", "he", "she", "it", "we", "they"}
    ...
    def _analyze_sentiment(self, text: str) -> str:
        ...
        positive_words = {"good", "great", "excellent", "positive", "wonderful", "fantastic", "amazing", "love", "like", "best", "happy", "joy", "success", "beautiful"}
        negative_words = {"bad", "terrible", "awful", "negative", "horrible", "worst", "hate", "dislike", "sad", "angry", "fail", "failure", "ugly"}
